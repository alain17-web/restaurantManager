...
import {DataTableFinanceData, Finance} from "../../../types/types.ts";
import {confirmAlert} from "react-confirm-alert";
import axiosInstance from "../../../axios/axiosInstance.tsx";
import useUsername from "../../../hooks/username/useUsername.tsx";
import ForbidenDelete from "../forbidenDelete/ForbidenDelete.tsx";

const DataTableFinances = (props: DataTableFinanceData) => {

    const [financeSummaries, setFinanceSummaries] = useState<Finance[]>([]);

    useEffect(() => {
        setFinanceSummaries(props.financeSummaries)
    }, [props.financeSummaries])
    ...
    const handleDelete = (id: number): void => {
        confirmAlert({
            title: 'Confirmation',
            message: 'Etes-vous sûr de vouloir supprimer ce rappprt ?',
            buttons: [
                {
                    label: 'Oui',
                    onClick: async () => {
                        try {
                            ...
                            setFinanceSummaries((prevSummaries) => prevSummaries.filter((summary) => summary.id !== id))
                        } catch (error) {
            ...
            return (
                <div className={"flex items-center mt-3"}>
                    <div
                        ...
                        onClick={() => props.getFinanceSummaryId(params.row.id)} data-bs-toggle="tooltip"
    ...
    return (
        <div className={"h-[950px] p-4"}>
            ...
            <DataGrid
                rows={financeSummaries}
                columns={financesColumns.concat(actionColumn)}
