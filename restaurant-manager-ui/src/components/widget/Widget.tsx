    ...
    return (
        <div className={"flex justify-between flex-1 h-[100px] p-[10px] custom-shadow rounded-[10px]"}>
            <div className={"flex flex-col justify-between relative"}>
                ...
                <span ><Link className={"text-gray-500 hover:text-purple-600 text-sm font-inter"} to={data.title ==="COMMANDES CLIENTS" ? "/listOrders" : data.title === "REAPPROS FOURNISSEURS" ? "/listPurchases" : "/listBookings"}>{data.link}</Link></span>
            </div>
        </div>
    );
};
